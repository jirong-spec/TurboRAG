...
TQTurboProdPageLayout make_tq_turbo_prod_page_layout(const TQConfig& cfg) {
    ...
    // K: one nibble per head-dim element → D/2 bytes (rounds up for odd D)
    layout.k4_bytes_per_token_head     = (cfg.head_dim + 1) / 2;
    ...
    const size_t k4_region         = token_heads * layout.k4_bytes_per_token_head;
    ...
    layout.k4_codes_offset = cursor; cursor += k4_region;
